...
namespace JumpArena
{
    ...
    {
        private TextBox _amountEditingTextBox;
        ...
        public frmPayments(int invoiceId, string customerName, string phone, decimal total, decimal discount, string voucherCode)
        {
            InitializeComponent();
            ...
            lblCustomer.Text = string.IsNullOrWhiteSpace(_customerName) ? "DEFAULT CUSTOMER" : _customerName;
        ...
        private void DgvPayments_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            // Gỡ handler cũ để không bị gắn chồng nhiều lần
            if (_amountEditingTextBox != null)
            {
                _amountEditingTextBox.TextChanged -= AmountEditingTextBox_TextChanged;
                _amountEditingTextBox.KeyPress -= Amount_KeyPress;
                _amountEditingTextBox = null;
            }

            // Chỉ xử lý khi đang edit cột Amount
            if (dgvPayments.CurrentCell.OwningColumn.Name == "Amount")
            {
                if (e.Control is TextBox tb)
                {
                    _amountEditingTextBox = tb;

                    // chỉ cho gõ số
                    tb.KeyPress += Amount_KeyPress;

                    // cập nhật liên tục khi gõ
                    tb.TextChanged += AmountEditingTextBox_TextChanged;
                }
            }
        }
        private void AmountEditingTextBox_TextChanged(object sender, EventArgs e)
        {
            if (dgvPayments.CurrentCell == null) return;
            if (dgvPayments.CurrentCell.OwningColumn.Name != "Amount") return;
            ...
            // lấy đúng số, bỏ dấu . , và ký tự linh tinh
            string raw = new string(txt.Where(char.IsDigit).ToArray());

            if (!decimal.TryParse(raw, out var amount))
                amount = 0;

            // set thẳng số vào cell để SumCollected đọc chuẩn
        ...
        private decimal SumCollected()
        {
            ...
            foreach (DataGridViewRow r in dgvPayments.Rows)
            {
                ...
                {
                    string raw = v.ToString().Replace(".", "").Replace(",", "").Trim();
...
VALUES (@Amount, GETDATE(), @Code, @Note, @InvoiceID, @PMID);", conn, tran))
                            ...
                            decimal originalTotal = _total + _discount; // _total là PAYABLE, discount là voucher
...
WHERE InvoiceID=@id;", conn, tran))
                            {
                                ...
                                upd.Parameters.AddWithValue("@Original", originalTotal);
                                upd.Parameters.AddWithValue("@Discount", _discount);
                                upd.ExecuteNonQuery();
                            }
    ...
    .ConnectionStrings["JumpArenaContextDB"].ConnectionString;
               ...
               using (var inv = new JumpArena.View.frmInvoice(_cs, _invoiceId, _customerName, _phone))
{
    inv.ShowDialog();
}
