...
namespace JumpArena
{
    ...
    {
        private decimal _voucherDiscountAmount = 0m;
        ...
        private void HomePage_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (Session.UserId > 0)
            {
                ShiftService.EndShift(Session.UserId);
                Session.Clear();
            }
        }

        private void ApplyVoucherToHome(decimal discountAmount, string voucherCode)
        {
            _voucherDiscountAmount = discountAmount < 0 ? 0 : discountAmount;
        ...
        {
            if (Session.UserId <= 0)
                throw new Exception("Session.UserId = 0. Login chưa set Session.");
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                string query = @"
INSERT INTO Invoice
(CreatedDate, TotalOriginalAmount, DiscountAmount, Status, PaymentStatus, UserID)
VALUES
(GETDATE(), 0, 0, 'Pending', 'Unpaid', @UserID);
...
SELECT CAST(SCOPE_IDENTITY() AS INT);";
        ...
        private void ResetBillUIOnly()
        {
            ...
            // DataSource có thể chưa set hoặc không đúng BindingList -> tránh ép kiểu chết
            if (dgvOrderItems.DataSource is BindingList<OrderItem> bl)
            {
                bl.ResetBindings();
            }
            else
            ...
            // Tránh NullReference nếu control bị rename/mất trong Designer
            if (txtCustomerName != null) txtCustomerName.Text = "";
            if (txtCustomerPhone != null) txtCustomerPhone.Text = "";

            _voucherDiscountAmount = 0m;
            _voucherCode = "";
...
WHERE InvoiceID = @id;", conn))
        ...
        private void BtnVoucher_Click(object sender, EventArgs e)
        {
            if (currentRoleID == 1 || currentRoleID == 2)
            {
                using (var f = new frm_Voucher(connectionString, orderItems))
                {
                    if (f.ShowDialog() == DialogResult.OK)
                    {
                        // Lưu lại voucher đang áp
                        _voucherDiscountAmount = f.AppliedDiscountAmount;
                        _voucherCode = f.AppliedVoucherCode;

                        UpdateOrderSummary(); // để Bill Total/Discount/Payable cập nhật
                    }
                }
                return;
            }
        ...
        private void BtnReport_Click(object sender, EventArgs e)
        {
            ...
            {
                OpenForm(typeof(ReportInboxForm), "Báo cáo và phản hồi");
            }
            else
            {
                ShowNoPermissionMessage();
        ...
        private void OpenForm(Type formType, string formName)
        {
            ...
            {
                // Không cho mở trùng
                if (_openedForms.TryGetValue(formType, out var existing) && existing != null && !existing.IsDisposed)
                {
                ...
                // nhớ lại để chống trùng
                _openedForms[formType] = form;

                // khi đóng thì remove khỏi cache
                form.FormClosed += (s, e) =>
                {
                ...
                // ShowDialog: HomePage tự bị disable, đúng yêu cầu
                form.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void LoadProducts()
        {
            allProducts.Clear();
            string query = @"
SELECT p.ProductID, p.ProductName, p.Description, p.Price, p.StockQuantity, p.IsStockManaged, c.CategoryID, c.CategoryName
FROM Product p
...
ORDER BY p.ProductID;";
...

                    {
                        while (reader.Read())
                        {
                            allProducts.Add(new ProductData
                            {
                                ...
                                IsStockManaged = reader.GetBoolean(5),
        ...
        private void UpdateOrderSummary()
        {
            ...
            decimal discount = _voucherDiscountAmount;
        ...
        private void EnsureInvoiceDetailsSaved(SqlConnection conn, SqlTransaction tran)
        {
...
WHERE InvoiceID=@InvoiceID", conn, tran))
        ...
        private void CreateNewInvoiceAndResetUI(bool askConfirmIfCurrentHasData)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                string query = @"
INSERT INTO Invoice (CreatedDate, TotalOriginalAmount, DiscountAmount, Status, PaymentStatus, UserID)
VALUES (GETDATE(), 0, 0, 'Pending', 'Unpaid', @UserID);
SELECT CAST(SCOPE_IDENTITY() AS INT);";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
        ...
        private bool IsNonStockProduct(string categoryName)
        {
            if (string.IsNullOrWhiteSpace(categoryName)) return false;

            string x = categoryName.Trim().ToLower();

            // Vé, Combo: không quản tồn kho
            return x.Contains("vé") || x.Contains("combo");
        }

        private string GetCategoryNameByProductId(int productId)
        {
            var p = allProducts.FirstOrDefault(z => z.ProductID == productId);
        ...
        private void BtnPayment_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtCustomerName.Text) ||
                string.IsNullOrWhiteSpace(txtCustomerPhone.Text))
            {
                MessageBox.Show("Nhập đầy đủ tên + SĐT khách!");
                return;
            }

            decimal totalOriginal = orderItems.Sum(x => x.Total);
            decimal payable = totalOriginal - _voucherDiscountAmount;
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    EnsureInvoiceDetailsSaved(conn, tran); // giờ đã lưu cả TotalOriginalAmount + DiscountAmount
                    tran.Commit();
                }
                catch
                {
            ...
            using (var f = new frmPayments(
                currentInvoiceID,
                txtCustomerName.Text.Trim(),
                txtCustomerPhone.Text.Trim(),
                payable,
                _voucherDiscountAmount,   // thêm
                _voucherCode              // thêm
            ))
            {
                var rs = f.ShowDialog();

                // ===== CHỈ TRỪ KHO KHI PAYMENT OK =====
                if (rs == DialogResult.OK)
                {
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        try
                        {
                            ...
                            // 2) Trừ kho (DÙNG CHUNG tran)
                            DeductInventoryAfterPayment(conn, tran);

                            // 3) Update trạng thái invoice (DÙNG CHUNG tran)
                            using (SqlCommand cmd = new SqlCommand(@"
UPDATE Invoice
...
WHERE InvoiceID=@id", conn, tran))
                            ...
                            // 4) Commit đúng 1 lần, ở đúng 1 chỗ
                            tran.Commit();

                            // 5) Sau commit: tạo invoice mới (KHÔNG dùng tran cũ nữa)
                            currentInvoiceID = CreateNewInvoicePending();
                            txtBillNo.Text = currentInvoiceID.ToString("D4");
                            ResetBillUIOnly();

                            // (tuỳ bạn) refresh list sản phẩm để stock trên UI cập nhật
                            LoadProducts();
                            DisplayPage();
                        }
                        catch (Exception ex)
        ...
        private void DeductInventoryAfterPayment(SqlConnection conn, SqlTransaction tran)
        {
            foreach (var item in orderItems)
            {
                ...
                // Vé: IsStockManaged = 0 => bỏ qua
                // Combo: IsStockManaged = 1 => bắt buộc trừ
                if (!product.IsStockManaged) continue;

                using (SqlCommand cmd = new SqlCommand(@"
UPDATE Product
SET StockQuantity = StockQuantity - @qty
WHERE ProductID = @pid
AND IsStockManaged = 1
...
", conn, tran))
                {
                    ...
                    if (affected == 0)
                        throw new Exception("Không đủ tồn kho: " + product.ProductName);
                }
            }
        }
